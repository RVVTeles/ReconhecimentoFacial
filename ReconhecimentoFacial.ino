...
const char* password = "TesteReconhecimentoFacial";
...
unsigned long face_recognition_block_until = 0;
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (millis() - interval > door_opened_millis) {
      digitalWrite(relay_pin, LOW); // Close relay after interval
    }

    fb = esp_camera_fb_get();

    // Always stream the video
    client.sendBinary((const char *)fb->buf, fb->len);

    // Block face detection during the cooldown period
    if (millis() >= face_recognition_block_until) {
          ...
          if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK) {
            ...
            if (g_state == START_RECOGNITION && (st_face_list.count > 0)) {
              ...
              if (f) {
                ...
                // Block further face recognition for 5 seconds
                face_recognition_block_until = millis() + 5000;
              } else {
